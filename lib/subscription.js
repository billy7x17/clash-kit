...
export async function useProfile(name) {
  ...
  const mergedConfig = {
    ...subscriptionConfig,
    'port': existingConfig['port'] || subscriptionConfig['port'],
    'bind-address': existingConfig['bind-address'] || subscriptionConfig['bind-address'],
    'socks-port': existingConfig['socks-port'] || subscriptionConfig['socks-port'],
    'allow-lan': existingConfig['allow-lan'] !== undefined ? existingConfig['allow-lan'] : subscriptionConfig['allow-lan'],
    'dns': existingConfig['dns'] || defaultDNS,  // 优先使用现有配置，否则使用默认安全配置
    'external-controller': existingConfig['external-controller'] || subscriptionConfig['external-controller'],
    'secret': existingConfig['secret'] || subscriptionConfig['secret'],
  }
